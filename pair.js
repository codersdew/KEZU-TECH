...
const config = {
  ...
  RCD_IMAGE_PATH: 'https://i.ibb.co/mF93Rzmh/tourl-1768827453121.jpg',
  NEWSLETTER_JID: '120363419143844721@newsletter',
  OTP_EXPIRY: 300000,
  OWNER_NUMBER: process.env.OWNER_NUMBER || '94779885727',
  CHANNEL_LINK: 'https://whatsapp.com/channel/0029Vb4uURW84OmD7Nxjbx2q',
  BOT_NAME: '💚𝐁𝐄𝐒𝐓𝐈𝐄_𝐌𝐈𝐍𝐈😘',
  BOT_VERSION: '1.0.0V',
  OWNER_NAME: 'bestie',
  IMAGE_PATH: 'https://ibb.co/39nkqdMM',
  BOT_FOOTER: '💚𝐁𝐄𝐒𝐓𝐈𝐄_𝐌𝐈𝐍𝐈😘',
  BUTTON_IMAGES: { ALIVE: 'https://i.ibb.co/mF93Rzmh/tourl-1768827453121.jpg' }
};
...
const MONGO_DB = process.env.MONGO_DB || 'BESTIEX2';
...
function setupCommandHandlers(socket, number) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    const args = body.trim().split(/ +/).slice(1);
        const reply = (text) => socket.sendMessage(msg.key.remoteJid, { text }, { quoted: msg });
...
`.trim();
    ...
    // Image
    const defaultImg = 'https://i.ibb.co/mF93Rzmh/tourl-1768827453121.jpg'; 
    const useLogo = userCfg.logo || defaultImg;
...
`.trim();
  ...
  await socket.sendMessage(sender, {
      image: { url: 'https://i.ibb.co/mF93Rzmh/tourl-1768827453121.jpg' }, // Simple Image msg for sub-menu or use Document if preferred
      caption: text,
      buttons,
...
`.trim();
  ...
  await socket.sendMessage(sender, {
      image: { url: 'https://i.ibb.co/mF93Rzmh/tourl-1768827453121.jpg' },
      caption: text,
      buttons,
      headerType: 1
  }, { quoted: msg });
...
case 'alive': {
    try {
        ...
        const logo = cfg.logo || config.RCD_IMAGE_PATH || 'https://i.ibb.co/mF93Rzmh/tourl-1768827453121.jpg'; // Default Image
...
╰═════════════╯`
        ...
        await socket.sendMessage(sender, {
            document: fs.readFileSync(__dirname + '/xion.docx'),
...
case 'gpt': {
  try {
    // 1. පරිශීලකයා එවූ පණිවිඩය ලබා ගැනීම
    const text = (msg.message.conversation || msg.message.extendedTextMessage?.text || '').trim();
    const q = text.split(" ").slice(1).join(" ").trim();

    // පණිවිඩයක් නොමැති නම්
    if (!q) {
      await socket.sendMessage(sender, { 
        text: '*🚫 කරුණාකර AI වෙත යොමු කිරීමට පණිවිඩයක් ඇතුළත් කරන්න.*',
        buttons: [
    ...
    // 2. බොට්ගේ නම සහ වින්‍යාසයන් (Settings) ලබා ගැනීම
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || 'KING KEZU';

    // 3. Meta AI විලාසිතාවේ Quote එකක් සෑදීම
    const metaQuote = {
    ...
    // 4. රියැක්ෂන් සහ Thinking පණිවිඩය
    await socket.sendMessage(sender, { react: { text: '🤖', key: msg.key } });
    const thinkingMsg = await socket.sendMessage(sender, { text: '*⏳ AI සිතමින් පවතී...*', quoted: metaQuote });

    // 5. Gemini API සඳහා ප්‍රොම්ප්ට් එක සකස් කිරීම
    const prompt = `
ඔබ සැබෑ ගැහැනු ලමයෙකු මෙන් හැසිරිය යුතුය. 
පිළිතුරු සැපයීමේදී ඉතා කෙටි සහ ආකර්ෂණීය ලෙස පිළිතුරු දෙන්න. 
ඉමෝජි පමනක් තිබේනම් ඔබද ඉමෝජි පමනක් යවන්න.
ඔයාගේ නම ${botName}. 
යමෙකු "ඔයාව හැදුවේ කවුද?" හෝ "Who created you?" ලෙස ඇහුවොත් පමණක් "මාව හැදුවේ KING KEZU" ලෙස පවසන්න.
User Message: ${q}
    `;

    // 6. Gemini 2.0 Flash API වෙත Request එක යැවීම
    const apiKey = "AIzaSyDD79CzhemWoS4WXoMTpZcs8g0fWNytNug"; // ඔබේ API Key එක
    const payload = {
      contents: [{
        parts: [{ text: prompt }]
      }]
    ...
    // පිළිතුර පරීක්ෂා කිරීම
    const aiReply = response.data?.candidates?.[0]?.content?.parts?.[0]?.text;

    if (!aiReply) {
      await socket.sendMessage(sender, { text: '*🚩 AI පිළිතුරක් ලබා දීමට අපොහොසත් විය.*' });
      return;
    }

    // 7. "Thinking" පණිවිඩය මකා අවසන් පිළිතුර යැවීම (සමහර Bot වල editMessage භාවිතා කළ හැක)
    await socket.sendMessage(sender, {
      text: aiReply,
      contextInfo: {
        externalAdReply: {
          title: botName,
          body: "Gemini 2.0 Flash AI",
          mediaType: 1,
          sourceUrl: "https://github.com/ChaminduRansika", // ඔබේ ලින්ක් එකක්
          thumbnailUrl: "https://i.ibb.co/3Yx4K9v/bot-thumb.jpg" // බොට්ගේ පින්තූරයක්
        }
      },
      ...
      headerType: 1,
      quoted: msg // පරිශීලකයාගේ පණිවිඩයට කෙලින්ම reply කිරීම
    });

  } catch (err) {
    console.error("Gemini AI Error:", err);
    await socket.sendMessage(sender, { 
      text: '*❌ AI පද්ධතියේ දෝෂයක් පවතී. පසුව උත්සාහ කරන්න.*'
    });
  }
  break;
}
...
//⚋⚋⚋⚋⚋⚋⚋⚋⚋pair case⚋⚋⚋⚋⚋⚋⚋⚋⚋⚋
    ...
    try {
        ...
        await socket.sendMessage(sender, {
            text: `> *𝐁𝚎𝚜𝚝𝚒𝚎 𝐌𝚒𝚗𝚒 𝐁𝚘𝚝 𝐏𝚊𝚒𝚛 𝐂𝚘𝚍𝚎 𝐒𝚄𝙲𝙲𝙴𝚂𝚂* ✅\n\n*🔑 Your pairing code is:* ${result.code}`
        }, { quoted: msg });
...
case 'cfn': {
  ...
  if (!jid || !jid.endsWith('@newsletter')) {
    await socket.sendMessage(sender, { text: '❗ Invalid JID. Example: xxxxxxxxxx@newsletter' }, { quoted: msg });
    break;
  }
...
case 'ping': {
    try {
        ...
        const cfg = await loadUserConfigFromMongo(sanitized) || {};
        const botName = cfg.botName || ' 💚𝐁𝐄𝐒𝐓𝐈𝐄_𝐌𝐈𝐍𝐈😘';
        const logo = 'https://i.ibb.co/mF93Rzmh/tourl-1768827453121.jpg'; // ඔයාගෙ ලින්ක් එක

        // 3. Determine Speed Status
        let speedStatus = '';
        if (ping < 200) speedStatus = '🚀 SUPERSONIC';
        else if (ping < 400) speedStatus = '🏎️ FAST';
...
`;
    ...
    try {
        ...
        // Preview Image URL
        const previewImgUrl = 'https://i.ibb.co/mF93Rzmh/tourl-1768827453121.jpg';
        ...
        await socket.sendMessage(sender, {
            ...
            contextInfo: {
                externalAdReply: {
                    ...
                    sourceUrl: "https://whatsapp.com/channel/0029Vb4uURW84OmD7Nxjbx2q", // Your channel link
                    mediaType: 1,
                    renderLargerThumbnail: true
                }
            }
...
╰───────────────╯
╭═══════════════╮
║ ▢ *OWNER* : ${config.OWNER_NAME || '💚𝐁𝐄𝐒𝐓𝐈𝐄_𝐌𝐈𝐍𝐈😘'}
...
case 'getdp': {
    try {
        ...
        try {
            ...
            ppUrl = "https://i.ibb.co/mF93Rzmh/tourl-1768827453121.jpg"; // default dp
        }
        ...
        await socket.sendMessage(sender, { 
            ...
            contextInfo: {
                externalAdReply: {
                    ...
                    thumbnailUrl: mediaUrl.match(/\.(jpeg|jpg|gif|png)$/) ? mediaUrl : "https://i.ibb.co/mF93Rzmh/tourl-1768827453121.jpg",
                    sourceUrl: mediaUrl,
                    mediaType: 1,
                    renderLargerThumbnail: true
                }
...
case 'tagall': {
  try {
    ...
    let groupPP = 'https://i.ibb.co/mF93Rzmh/tourl-1768827453121.jpg';
